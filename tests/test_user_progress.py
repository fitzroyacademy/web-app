...
from enums import SegmentPermissionEnum, SegmentStatus
...
class TestUserProgress(ObjectsGenerator, unittest.TestCase):
    def setUp(self):
        with app.app_context():
            ...
            self.l1s1 = self.make_segment(self.l1, title="Intro segment l1", order=0,
                                          slug="intro-segment-l1", type="text")
            self.l1s2 = self.make_segment(self.l1, title="Segment l1s1", order=1, slug="segment-1")
            self.l1s3 = self.make_segment(self.l1, title="Segment l1s2", order=2, slug="segment-2")
            ...
            self.l2s1 = self.make_segment(self.l2, title="Intro segment l2", order=0,
                                          slug="intro-segment-l2", type="text")
            self.l2s2 = self.make_segment(self.l2, title="Segment l2s1", order=1, slug="segment-1")
            self.l2s3 = self.make_segment(self.l2, title="Segment l2s2", order=2, slug="segment-2")
    ...
    def test_soft_barrier_not_done(self):
        ...
        self.assertEqual(self.l1s2.user_status(self.user), SegmentStatus.accessible)  # we should be able to make barrier segment
    ...
    def test_soft_barrier_done(self):
        ...
        progress = datamodels.SegmentUserProgress(progress=97, segment_id=self.l1s2.id, user_id=self.user.id)
    ...
    def test_soft_barrier_blocking(self):
        ...
        progress1 = datamodels.SegmentUserProgress(progress=98, segment_id=self.l0s0.id, user_id=self.user.id)
        progress2 = datamodels.SegmentUserProgress(progress=57, segment_id=self.l1s1.id, user_id=self.user.id)
    ...
    def test_hard_barrier_completed(self):
        ...
        progress1 = datamodels.SegmentUserProgress(progress=98, segment_id=self.l0s0.id, user_id=self.user.id)
        progress2 = datamodels.SegmentUserProgress(progress=99, segment_id=self.l1s1.id, user_id=self.user.id)
        self.session.add(progress1)
        self.session.add(progress2)
        self.session.commit()
        self.assertEqual(self.l0s0.user_status(self.user), SegmentStatus.completed)
        ...
        progress1 = datamodels.SegmentUserProgress(progress=98, segment_id=self.l1s2.id, user_id=self.user.id)
        self.session.add(progress1)
        self.session.commit()
        self.assertEqual(self.l1s2.user_status(self.user), SegmentStatus.completed)
        self.assertEqual(self.l1s3.user_status(self.user), SegmentStatus.accessible)
