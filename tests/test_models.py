...
class TestModels(unittest.TestCase):
    ...
    def test_course_progress_aggregate_by_user(self):
        ...
        self.session.add(course)
        lesson = datamodels.Lesson(course=course, **self.standard_course_lesson)
        lesson_b = datamodels.Lesson(course=course, **self.standard_course_lesson)
        self.session.add(lesson)
        ...
        seg_a = datamodels.Segment(lesson=lesson, **self.standard_course_lesson_segment)
        seg_b = datamodels.Segment(lesson=lesson, **self.standard_course_lesson_segment)
        seg_c = datamodels.Segment(lesson=lesson_b, **self.standard_course_lesson_segment)
        seg_d = datamodels.Segment(lesson=lesson_b, **self.standard_course_lesson_segment)
        self.session.add(seg_a)
        self.session.add(seg_b)
    ...
    def test_user_total_course_progress(self):
        ...
        lesson = datamodels.Lesson(course=course, **self.standard_course_lesson)
        lesson_b = datamodels.Lesson(course=course_b, **self.standard_course_lesson)
        self.session.add(lesson)
        self.session.add(lesson_b)
        user = self.makeUser()
        self.session.add(user)
